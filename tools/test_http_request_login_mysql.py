...
test_data = DoMysql().get_data("login")#执行所有用例
my_logger = MyLog()
...
class TestHttpRequest_Login(unittest.TestCase):
    ...
    #     res = HttpRequest.http_request(item['url'],item['data'],item["header"],item["params"],item['http_method'],getattr(GetData,"Cookie"))
        if res.cookies:
